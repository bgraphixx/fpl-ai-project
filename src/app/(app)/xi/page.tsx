...
export default async function XIPage() {
  const session = await auth();
  if (!session?.user) redirect("/login");
  const result = await loadSquad(session.user.id);

  if (result.kind === "ok") return <XIFlow squad={result.squad} />;
  ...
  if (result.kind === "not-linked") {
    return (
      <StateScreen
        ...
        message="Link your FPL Team ID so the AI can work with your real squad."
