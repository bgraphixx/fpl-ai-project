...
import { buildRecommendationEmail } from "@/lib/email-templates";
...
export async function POST(request: Request) {
  ...
  // Fire-and-forget build recommendation email
  if (session.user.email) {
    ...
    const { subject, html } = buildRecommendationEmail(session.user.email, gameweek, formation, captainName, aiResult.summary);
